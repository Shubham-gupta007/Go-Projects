...
func main() {
	...
	if name == `NAME="CentOS Linux"` {
		...
		if string(checkFileExistsResponse) == "" {
			...
			mongoFilesMoveCommand := "sudo mv mongod.conf mongod_arb.conf mongodb-org.repo sslkey mongoreplicaa.json /opt/zona/mongosetup"
			...
			//5.1 Mkdir mongodb_Arb
			makeArbDirMongoCommand := "sudo mkdir /var/lib/mongodb_arb"
			makearbdirMongo, err := exec.Command("sh", "-c", makeArbDirMongoCommand).Output()
			if err != nil {
				fmt.Println("Error in Executing make arb dir:" + err.Error())
				setupLog("Error in Executing make arb dir:" + err.Error())
			} else {
				if string(makearbdirMongo) == "" {
					fmt.Println("Make Arb dir Created")
					setupLog("Make Arb dir Created")
				}
			}
			...
			//8.1 Move mongod_arb.conf file
			mongoArbConfMoveCommand := "sudo mv /opt/zona/mongosetup/mongod_arb.conf /etc/mongod_arb.conf"
			mongoArbConfMoveFile, err := exec.Command("sh", "-c", mongoArbConfMoveCommand).Output()
			if err != nil {
				fmt.Println("Error in Executing moving arb conf Command:" + err.Error())
				setupLog("Error in Executing moving arb conf Command:" + err.Error())
			} else {
				if string(mongoArbConfMoveFile) == "" {
					fmt.Println("Mongo Arb Conf File Moved ")
					setupLog("Mongo Arb Conf File Moved ")
				}
			}

			//8.2 Move sslkey file
			...
			//11.1 Start mongodb_Arb
			startArbMongoCommand := "sudo mongod --config /etc/mongod_arb.conf"
			startarbMongo, err := exec.Command("sh", "-c", startArbMongoCommand).Output()
			if err != nil {
				fmt.Println("Error in Executing arb start Mongodb:" + err.Error())
				setupLog("Error in Executing arb start Mongodb:" + err.Error())
			} else {
				if string(startarbMongo) == "" {
					fmt.Println("Mongodb Arb Started")
					setupLog("Mongodb Arb Started")
				}
			}
			...
			// primaryhost := host[:strings.IndexByte(host, ':')]

			//Update /etc/hosts file to add securaa
			addHostCommand := `sudo echo ` + host + ` ` + `securaa` + ` >> /etc/hosts`
