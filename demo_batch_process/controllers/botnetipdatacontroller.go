...
type (
	BotnetIPDataController struct{}
)

func NewBotnetIPDataController() *BotnetIPDataController {
	return &BotnetIPDataController{}
}

func (ip BotnetIPDataController) GetBotnetIPInformation(configObject config.ConfigStruct, client *elastic.Client) {
	logObj := logger.ErrorLog{}
	logObj.Source = "abusech"
	logObj.LogFileName = "botnetipdatacontroller.go"
	logObj.LogFunctionName = "GetBotnetIPInformation"

	data, responseCode := common.GetRestData(constants.Abuse_botnetipblocklist, logObj)
	if responseCode == 200 {
			...
			if len(eachRecord) > 1 {
				...
				if !strings.HasPrefix(eachRecord[0], "#") {
					...
					eachColumn := models.IndicatorData{
						...
						SourceLink:  constants.Abuse_botnetipblocklist,
		...
		logObj.BatchLog("-----------------------------")
		logObj.BatchLog("NO DATA FROM " + constants.Abuse_botnetipblocklist + ". RESPONSE CODE : " + strconv.Itoa(responseCode))
		logObj.BatchLog("-----------------------------")
	}
}
