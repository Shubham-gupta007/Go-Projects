...
type (
	IPDataController struct{}
)

func NewIPDataController() *IPDataController {
	return &IPDataController{}
}

func (ip IPDataController) GetIPInformation(configObject config.ConfigStruct, client *elastic.Client) {
	logObj := logger.ErrorLog{}
	logObj.Source = "abusech"
	logObj.LogFileName = "ipdatacontroller.go"
	logObj.LogFunctionName = "GetIPInformation"

	data, responseCode := common.GetRestData(constants.Abuse_ipblocklist, logObj)
	if responseCode == 200 {
			...
			if len(eachRecord) > 1 {
				...
				if !strings.HasPrefix(eachRecord[0], "#") {
					...
					eachColumn := models.IndicatorData{
						...
						SourceLink:  constants.Abuse_ipblocklist,
						Malware:     malware,
		...
		logObj.BatchLog("-----------------------------")
		logObj.BatchLog("NO DATA FROM " + constants.Abuse_ipblocklist + ". RESPONSE CODE : " + strconv.Itoa(responseCode))
		logObj.BatchLog("-----------------------------")
	}
}
